...
def main():
    ...
    for i,avg_k in enumerate(avg_ks):
        for _ in range(L):
            ...
            G = generate_ws_graph(num_nodes, avg_k, beta)
            strategies = []
    ...
    for i,avg_k in enumerate(avg_ks):
        np.savetxt("v3_ws_robustness_N_{}_L_{}_beta_{:.3f}_k_{}.dat".format(num_nodes, L, beta, avg_k), np.array([fs, Prandom[i,:], Pdegree[i,:], Pbetweenness[i,:], Pcloseness[i,:]]).T)
