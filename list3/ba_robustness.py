...
def main():
    ...
    for i,avg_k in enumerate(avg_ks):
        for _ in range(L):
            ...
            Gdegree = generate_ba_graph(num_nodes, avg_k)
    ...
    for i,avg_k in enumerate(avg_ks):
        np.savetxt("v2_ba_robustness_N_{}_L_{}_k_{}.dat".format(num_nodes, L, avg_k), np.array([fs, Prandom[i,:], Pdegree[i,:], Pbetweenness[i,:], Pcloseness[i,:]]).T)
